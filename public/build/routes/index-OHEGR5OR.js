...
var fetcher = (url3) => import_axios.default.get(url3, { withCredentials: true }).then((res) => res.data);
function useUser() {
  const { data, error } = useSWR(`${url}/users/verify`, fetcher);
  return {
    user: data,
...
async function sendRequestToBackend(method, route, data, primaryRoute) {
  try {
    const response = await (0, import_axios2.default)({
      method,
      url: `${url2}/${primaryRoute}${route ? `/${route}` : ""}`,
...
function Home() {
  ...
  const headPhones = data.data.data.headphones;
  const laptops = data.data.data.laptops;
  return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(import_jsx_dev_runtime.Fragment, {
    children: [
 ...
 * @license MIT
 */
//# sourceMappingURL=/build/routes/index-OHEGR5OR.js.map
